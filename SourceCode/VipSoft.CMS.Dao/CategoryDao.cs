    ...
    public class CategoryDao : AbstractDao<Category>, ICategoryDao
    {
        ...
        public List<Category> GetBrotherNode(int id)
        {
            ...
            {                
                ...
                var sql = string.Format(@"SELECT * FROM vipsoft_category A WHERE EXISTS (SELECT '1' FROM vipsoft_category B WHERE A.parent_id=B.parent_id AND A.STATUS=1 AND B.id={0}) ORDER BY sequence", parameter[0].ParameterName);
