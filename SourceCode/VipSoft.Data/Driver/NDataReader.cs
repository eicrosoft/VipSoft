...
namespace VipSoft.Data.Driver
{
    ...
    /// FSORM relies on being able to create MARS to read Components and entities inside
    /// of Collections.
    /// </summary>
    /// <remarks>
    ...
    public class NDataReader : IDataReader
    {
        ...
        /// <remarks>
        /// FSORM attempts to not have to read the contents of an <see cref="IDataReader"/> into memory until it absolutely
        ...
        public NDataReader(IDataReader reader, bool isMidstream)
        {
            ...
            {
                throw new Exception("There was a problem converting an IDataReader to NDataReader", e);
            }
            finally
            {
                reader.Close();
        ...
        private class NResult
        {
            ...
            public int GetOrdinal(string colName)
            {
                ...
                {
                    if (StringComparer.InvariantCultureIgnoreCase.Compare(pair.Key, colName) == 0)//StringHelper.EqualsCaseInsensitive(pair.Key, colName))
                    {
                        return pair.Value;
                    }
                }
